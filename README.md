...
Git config; `npm install` / `npm ci` wires them automatically via the `prepare` script (a portable Node guard, `scripts/prepare-merge-driver.ts`, the canonical pm-ops launcher template, copied unchanged and checked byte-for-byte by a test: it runs
`pm merge install` only when the `pm` CLI is on `PATH`, and no-ops cleanly when `pm` is absent. Registry installs of this package never run `prepare`; a production install of a clone (`npm ci --omit=dev`) omits `pm-ops` too, and the launcher then skips with one notice instead of failing; being Node-based it behaves identically
